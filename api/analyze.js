...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Leer como texto primero para manejar errores no-JSON de Anthropic
    const responseText = await r.text();
    let data;
    try {
      data = JSON.parse(responseText);
    } catch {
      console.log("ANTHROPIC RAW ERROR:", responseText.slice(0, 200));
      return res.status(502).json({ error: `API no disponible — reintentá en unos segundos (${responseText.slice(0, 60)})` });
    }
